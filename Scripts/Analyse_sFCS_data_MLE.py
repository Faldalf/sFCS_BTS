...
# Analyse sFCS transit time data for lognormal or double-lognormal behaviour indicating free or hindered diffusion mode.
...
# Double lognormal (dlogN) behaviour indicates a second component usually attributed to a population of molecules undergoing trapping (transient halts in the diffusion path).
#
# This is a cleaned-up version to accompany the BTS histogram manuscript. 
# The data to analyse should be in an excel or csv file. The column with the transit times should be called "txy1". 
# This is modelled after my standard workflow analysing raw intensity fluctuations using FoCuS_scan (https://github.com/dwaithe/FCS_scanning_correlator) and saving the fitting parameters to an excel sheet. 
#
# The data will be analysed using a maximum likelihood estimation (MLE). The script computes a relative likelihood value indicating which of the three models (Gauss, logN, dlogN) is the best model to represent the data. 
# -> If RL for dlogN is 1 it is likely that the diffusion behaviour is hindered.  
# sFCS data will not be Gaussian. This is somewhat a noise/sanity control.
#
...
# NLL_Gauss retruns the Negative logarithmic Likelihood value for Lognormal
# distributed data using theta1 for mu and theta2 for sigma. 
...
#  NLL_Gauss retruns the Negative logarithmic Likelihood value for 2 component / double lognormal
#  distributed data using theta1 for mu and theta2 for sigma. 
...
def model_selection_RL (data, initial_guess = [4,1], plot = 'on'):
    # Calculates the relative likelihood based on the functions above for data being normally or lognormally or double lognormally distributed
    ...
    # bounds (bnds) are provided hard-coded here ... that is not optimal but really helps with convergence. Probably need to be varied for special cases
    # 
    ...
    # Initialisation is a bit dirty and could be optimised using specific values not based on mu and sigma for logN. 
    ...
    # Let's save the interesting data 
